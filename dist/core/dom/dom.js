import { assignValue, date, getType, inArray, isArray, isBoolean, isCallable, isEmpty, isFunction, isNull, isNumber, isObject, isString, objectHasKey, _defineProperty, _instanceof, getArguments, getEl } from '../utils.js';
import createClass, { _class, createInstance, getClassData } from '../es5-class.js';
import app from '../app.js';
var global = window;
var MS = date('ms');
var DEFAULT_VALUE = Str.rand(Str.rand(MS));
var SHOW = 'SHOW' + Str.rand(Str.rand(MS));
var PENDING_CHILDREN = 'PENDING_CHILDREN' + Str.rand(Str.rand(MS));
var PENDING_CONTENTS = 'PENDING_CONTENTS' + Str.rand(Str.rand(MS));
var PARENT_NODE = 'PARENT_NODE' + Str.rand(Str.rand(MS));
var MARK_COMMENT = 'MARK_COMMENT' + Str.rand(Str.rand(MS));
var DATA_TYPES = 'DATA_TYPES' + Str.rand(Str.rand(MS));
var SYNC_CHANGE = 'SYNC_CHANGE' + Str.rand(Str.rand(MS));
var DATA_SYNC = 'DATA_SYNC' + Str.rand(Str.rand(MS));
var DYNAMIC_SYNC = 'DYNAMIC_SYNC' + Str.rand(Str.rand(MS));
var DYNAMIC_ATTRS = 'DYNAMIC_ATTRS' + Str.rand(Str.rand(MS));
var DATA_CONTAINERS = 'DATA_CONTAINERS' + Str.rand(Str.rand(MS));
var BUILDER = 'BUILDER' + Str.rand(Str.rand(MS));
var IS_STARTED = 'IS_STARTED' + Str.rand(Str.rand(MS));
var LISTENNERS = 'LISTENNERS' + Str.rand(Str.rand(MS));
var DOM_LISTENNERS = 'DOM_LISTENNERS' + Str.rand(Str.rand(MS));
var TRANSMISTION_LISTENNERS = 'TRANSMISTION_LISTENNERS' + Str.rand(Str.rand(MS));
var TRANSMISTION_STATUS = 'TRANSMISTION_STATUS' + Str.rand(Str.rand(MS));
var IS_BUILDED = 'IS_BUILDED' + Str.rand(Str.rand(MS));
var CAN_SET_CHILDREN = 'CAN_SET_CHILDREN' + Str.rand(Str.rand(MS));
...
var domEvents = ("blur focus focusin focusout resize scroll click dblclick " +
    ...
    "change select submit keydown keypress keyup contextmenu load input").split(" ");
var allEvents = ("abort afterprint animationend animationiteration animationstart beforeprint beforeunload blur canplay canplaythrough " +
    ...
    "touchcancel touchend touchmove touchstart transitionend unload volumechange waiting wheel").split(" ").filter(function (s) { return s.length > 0; });
...
var events = [];
var inputTypes = [
    "button", "submit", "image", "reset", "checkbox", "radio",
    "color", "range",
    "file", "hidden",
...
function closest(selector, element) {
    ...
    if (selector == undefined)
        return element ? element.parentNode || null : null;
    var finder = [];
    ...
    if (findLength) {
        ...
        for (var n = 0; n < parentLength; n++) {
            parent = parents[n];
            for (var m = 0; m < findLength; m++) {
                find = finder[m];
                if (parent == find) {
                    closestList = parent;
                    m += findLength;
...
var DomBag = /** @class */ (function () {
    function DomBag(_dom, args) {
        this.domClass = null;
        this.args = [];
        this.isDomBag = true;
        this.domClass = _dom;
    ...
    DomBag.prototype.make = function () {
        return createInstance(this.domClass, this.args);
    };
    DomBag.prototype.withParent = function (parent) {
...
function addDomClassData(instanceID, data) {
    if (typeof DomClassData[instanceID] == "undefined")
        DomClassData[instanceID] = {};
    if (data && isObject(data))
        assignValue(DomClassData, instanceID, data);
}
...
function setDomDataValue(instanceID, key, value) {
    ...
    if (isString(key))
        ...
        assignValue(DomClassData, instanceID, key);
}
function getDomClassPendingDAta(classCTX) {
    for (var index = 0; index < DomPendingClassData.length; index++) {
        var wrapper = DomPendingClassData[index];
        if (wrapper.$class == classCTX)
            return wrapper.data;
    }
    var w = {};
...
Dom = _class("Dom")({
    ...
    __prepare__: function (props, scope, resolve, reject, classCTX) {
        ...
        if (hasData) {
            for (var index = 0; index < dataContainers.length; index++) {
                var container = dataContainers[index];
                if (container._class == c) {
    ...
    __boot__: function () {
        ...
        if (oneTimeData && isObject(oneTimeData) && !isEmpty(oneTimeData)) {
            for (var key in oneTimeData) {
                if (Object.hasOwnProperty.call(oneTimeData, key)) {
                    var value = oneTimeData[key];
                    this[key] = value;
                }
            }
            oneTimeData = {};
                ...
                if (this.__get__(DYNAMIC_SYNC)) {
                    ...
                    for (var key in __dynamicAttrs) {
                        if (Object.hasOwnProperty.call(dyna1micAttrs, key)) {
                            ...
                            var camelKey = Str.slugToCamel(key), slugKey = Str.camelToSlug(key);
                            var s = null;
                            var v = null;
    ...
    const$setElement: function setElement(params) {
        ...
        if (el) {
            ...
            if (!isEmpty(elem.dataTypeAttrs)) {
                for (var key in elem.dataTypeAttrs) {
                    if (Object.hasOwnProperty.call(elem.dataTypeAttrs, key)) {
                        var vl = elem.dataTypeAttrs[key];
    ...
    const$getDefaultSelector: function () {
        ...
        return "div#" + (this.id ? this.id : Str.rand()) + "." + (this.className ? this.className.split(" ").map(function (v) {
            return v.trim();
        }).filter(function (v) {
            return v.length > 0;
        }).join('.') : 'dom-element');
    ...
    addEventListener: function (type, handler, passed) {
        ...
        if (_listeners === undefined || _listeners === null)
            _listeners = {};
        type = String(type).toLowerCase();
        if (isDomEvent(type))
            return this.addDomEvent(type, listener, DEFAULT_VALUE);
    ...
    removeEventListener: function (type, listener) {
        ...
        type = String(type).toLowerCase();
        if (isDomEvent(type))
            return this.off.apply(getArguments(arguments));
        var listeners = _listeners;
    ...
    dispatchEvent: function (event) {
        ...
        if (_listeners === undefined || _listeners === null)
            return;
        if (isDomEvent(event.type))
            return this.trigger.apply(this, getArguments(arguments));
        var listeners = _listeners;
    ...
    addDomEvent: function (event, handler, passed) {
        ...
        var listener = passed === DEFAULT_VALUE ? handler : this.parseEventHandler(handler);
        if (!isDomEvent(event))
            return this.addEventListener(event, listener, DEFAULT_VALUE);
        var _domlisteners = this.__get__(DOM_LISTENNERS);
        event.split(" ").filter(function (evt) { return (allEvents.indexOf(evt) >= 0); }).map(function (evt) {
            addEvent(element, evt, listener, null);
            if (_domlisteners.indexOf(evt) == -1)
                _domlisteners.push(evt);
        });
        return this;
    },
    ...
    const$fn: function fn(method) {
        var self = this;
        if (!isString(method) || typeof self[method] != "function")
        ...
        if (isString(selector)) {
            if (selector.length > 1 && selector.substring(0, 1) == ":") {
                ...
                if (_instanceof(e, Element))
                    return (typeof e[s] != 'undefined' && e[s]) ? true : false;
                for (var i = 0; i < e.length; i++) {
                    var ell = e[i];
                    if (typeof ell[s] != 'undefined' && ell[s])
                        return true;
                }
                return false;
            }
    ...
    getHtml: function () {
        if (!this.el || isGlobalOrRoot(this.el))
            return "";
    ...
    css: function (prop, value) {
        ...
        if (!this.el) {
            ...
            if (Object.hasOwnProperty.call(style, p))
                return style[p];
            if (Object.hasOwnProperty.call(style, s))
                return style[s];
            return "";
        }
        if (isString(prop)) {
    ...
    const$append: function (child) {
        if (typeof child == "undefined" || isNull(child) || simpleTags.indexOf(this.tagName) !== -1)
            return this;
        var self = this;
        ...
        if (isArray(child)) {
            ...
            if (child.isDom) {
                ...
                var c_1 = child.withParent(this);
                __build__.call(c_1);
                this.el.appendChild(c_1.el);
                this.children.push(c_1);
            }
        ...
        if (isObject(child)) {
            if (child.isDom) {
                ...
                for (var index = child.length - 1; index > -1; index--) {
                    var c_3 = child[index];
                    this.el.insertBefore(c_3, this.el.firstChild);
                }
                this.children.unshift(child);
            }
            else if (child instanceof Element) {
    ...
    final$removeChild: function (child, removeDomEl) {
        if (typeof removeDomEl == "undefined")
            removeDomEl = true;
        if (!isBoolean(removeDomEl))
            removeDomEl = true;
        if (child) {
            var self = this;
    ...
    final$stopCallChildrenMethod: function () {
        if (curremtCallChildrenMethodID)
            stopCallChildrenTask[curremtCallChildrenMethodID] = true;
    },
    /**
    ...
    final$getRootElement: function () {
        if (this.parent && isObject(this.parent) && this.parent.isDom)
    ...
    final$moveIn: function moveIn(child, pos, oldparent) {
        if (isObject(child)) {
            var t;
            if ((isBoolean(pos) && pos === true) || inArray(['top', 'start', 'prepend'], pos))
                t = true;
            else
                t = false;
            var self = this;
    ...
    set$children: function onSetChildren(value) {
        ...
        if (!csc)
            console.warn("Bạn không thể set giá trị cho children");
        return csc;
    },
...
function bootData() {
    var bag = getDataBag(this.static);
    var data = {};
    if (isObject(bag)) {
        for (var key in bag) {
            if (Object.prototype.hasOwnProperty.call(bag, key)) {
                var scopeData = bag[key];
                if (key == 'services') {
    ...
    var _data_containers = this.__get__(DATA_CONTAINERS);
    if (!isEmpty(_data_containers)) {
        for (var key in _data_containers) {
            if (Object.prototype.hasOwnProperty.call(_data_containers, key)) {
                var sc = _data_containers[key];
                if (key == 'servives') {
                    if (isObject(sc)) {
                        for (var k in sc) {
...
function receiveFromParent(channel, data, sentId) {
    ...
    if (next) {
        if (!a) {
            var c = Str.replace(Str.ucword(channel.split("-").join(" ")), " ", "");
            var listFn = [
...
function __callChildrenMethod__(method, args, className) {
    var instanceId = this.__instance__id__;
    if (!curremtCallChildrenMethodID)
        curremtCallChildrenMethodID = instanceId;
...
function ___assignDynamicProperties___() {
    ...
    function hide(time, callback) {
        ...
        if (!self.el.parentNode)
            return self;
        if (!self.__get__(PARENT_NODE))
            self.__set__(PARENT_NODE, self.el.parentNode);
    ...
    function next(args) {
        if (self.parent) {
            ...
            var s = false;
            for (var i = index + 1; i < self.parent.children.length; i++) {
                ...
                else if (isFunction(args) && args(child))
                    return child;
                else if (!args)
                    return child;
            }
...
function setDataTypeAttribute(key, value, sync) {
    if (!((isObject(value) && (value.isObjectData || value.isArrayData) || value.isPrimitive) || (isFunction(value) && value.isPrimitive)))
        return sync ? this.addDynamicAttr(key, value) : this.setAttribute(key, value);
...
function addDynamicAttr(attr, value) {
    if (isString(attr)) {
        var self = this;
        if ((isObject(value) && (value.isObjectData || value.isArrayData) || value.isPrimitive) || (isFunction(value) && value.isPrimitive))
            return setDataTypeAttribute.call(this, attr, value);
...
function create(tag, children, attributes) {
    var tagName = 'div', id = '', className = '', attrs = {}, __dynamicAttrs = {}, events = {}, props = {}, methods = {}, contents = [], contentf = "", inf = { isElement: false }, isSimple = false, isArrayContent = false, isTwoContent = 0, dataTypeAttrs = {}, bindingAttrs = {}, data = {}, parent = null;
    function addAttrValue(k, vl) {
        ...
        if (s.substr(0, 1) == '$') {
            ...
            for (var v_1 in vl) {
                if (Object.hasOwnProperty.call(vl, v_1)) {
                    var ev = vl[v_1];
                    events[v_1] = ev;
                }
            }
    ...
    if (!isEmpty(props)) {
        for (var key_1 in props) {
            if (Object.hasOwnProperty.call(props, key_1)) {
                var value = props[key_1];
                htmlObject[key_1] = value;
            }
        }
    }
...
var createEl = function createEl(tag) {
    ...
    var tagName = 'div', id = '', className = '', attrs = {}, __dynamicAttrs = {}, events = {}, props = {}, contents = [], contentf = "", inf = { isElement: false }, isSimple = false, boot = null, init = null;
    /**
    ...
    if (typeof boot == "function")
        boot.call(htmlObject, attrs, events);
    if (!isEmpty(attrs)) {
        var csk, v;
        var css = {};
        for (var prop in attrs) {
...
function addEventListener(el, event, handler) {
    if (!(el instanceof Element) && el != document && el != window)
        return false;
...
function getDomInf(s) {
    ...
    for (var i = 0; i < a.length; i++) {
        ...
        if (c == " ") {
            if (!obj.id && !obj.className && !obj.props && !obj.attrs && !obj.content)
                fail = true;
        }
        if (contentMode > 0) {
            if (c == "{") {
...
function addEvent(element, event, callback, data) {
    if (!element || !event || (!(element instanceof Element) && !isGlobalOrRoot(element)) || !isString(event) || !callback || !isCallable(callback))
        return false;
    event = event.toLowerCase();
    data = data || null;
    ...
    for (var i = 0; i < events.length; i++) {
        ...
        if (eventData.element === element && eventData.event === event) {
            for (var j = 0; j < eventData.tasks.length; j++) {
                ...
                if (data) {
                    if (data == evCallback.data && evCallback.callback === callback)
                        return true;
                }
                else if (evCallback.callback === callback)
                    return true;
            }
...
function isDomEvent(eventName) {
    ...
    if (isArray(eventName))
        eventName.map(function (e) { if (allEvents.indexOf(String(e).toLowerCase()) != -1)
            stt = true; });
    else if (isString(eventName) && allEvents.indexOf(eventName.toLowerCase()) != -1)
        stt = true;
    return stt;
}
;
...
function isDomBasicEvent(eventName) {
    var stt = false;
    if (isArray(eventName))
        eventName.map(function (e) { if (domEvents.indexOf(String(e).toLowerCase()) != -1)
            stt = true; });
    else if (isString(eventName) && domEvents.indexOf(eventName.toLowerCase()) != -1)
        stt = true;
    return stt;
}
;
...
function getParentNodes(elem, list, elementStop) {
    ...
    if (elem instanceof Element) {
        if (!elem.parentNode || (elementStop && elementStop == elem))
            return list;
...
function isQuery(obj) {
    if (!isObject(obj))
        return false;
    if (obj.isQuery || obj.isDomQuery)
        return true;
    return false;
}
function isHTML(str) {
    return isString(str) ? (/<(?=.*? .*?\/*>|br|hr|input|!--|wbr)[a-z\-_\:]+.*?>|<([a-z\-_\:]+).*?<\/\1>/i.test(str)) :
...
function emptyFunc() { }
export default Dom;
export { createEl, create, getDomInf, Dom, inputTypes, inputTags, htmlTags, domEvents };
